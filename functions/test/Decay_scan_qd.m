function Decay_scan_qd( op, epstab, d_length, typeStr, exc, bem, ene2, enei2, dip_p, dip_d, p, imp)
    multiWaitbar( 'FF spectra enhancement', 0, 'Color', 'g', 'CanCancel', 'on' ); %  Loop over wavelengths 
    ...
    for j=1:3
        for ien = 1 : length( enei )
            ...
            if j==1
                ff=farfield_qd(op, enei, ien, dip_d, spec); % Farfield    
            elseif j==2
            ...
            if j==1
                ff_s=farfield_qd(op, enei, ien,dip_d, spec_s); % Farfield      
            elseif j==2
            ...
            multiWaitbar( 'FF spectra enhancement', ien / numel( enei ));
    ...
    xlabel( 'Energy (eV)' );
    ylabel( 'L/Ld' );
    title( 'Angular FF spectra enhancement' );
