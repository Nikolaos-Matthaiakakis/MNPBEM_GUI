...
function CL_map(impact, ene2, enei2, spec_s, spec, xe, bem, exc_CL_M, xx, yy, Double_en, vel)
    ...
    misc.atomicunits;
    multiWaitbar( 'CL maps', 0, 'Color', 'g', 'CanCancel', 'on' );    
    for ien = 1 : length( enei2 ) %  Loop over energies
        ...
        CL_sca_m_CL(:,ien)=sca_m.*fine ^ 2 / ( bohr * hartree * pi * vel )./(2*(pi^2)./enei2(ien)); %  Farfield scattering spectra for whole sphere
        %---- P pol
        ff_mp=ff_m;
        ...
        CL_sca_m_CLp(:,ien)=sca_mp.*fine ^ 2 / ( bohr * hartree * pi * vel )./(2*(pi^2)./enei2(ien)); %  Farfield scattering spectra for whole sphere
        %---- S pol
        ff_ms=ff_m;
        ...
        CL_sca_m_CLs(:,ien)=sca_ms.*fine ^ 2 / ( bohr * hartree * pi * vel )./(2*(pi^2)./enei2(ien)); %  Farfield scattering spectra for whole sphere
        % Angular
        ff_ma=farfield(spec,sig); % Farfield for angular  
        [sca_ma,~] = scattering(ff_ma); %  Farfield scattering for Angular
        CL_sca_m_CLa(:,ien)=sca_ma.*fine ^ 2 / ( bohr * hartree * pi * vel )./(2*(pi^2)./enei2(ien)); %  Farfield scattering spectra for Angular
        %---- P pol
        ff_map=ff_ma;
        ...
        CL_sca_m_CLap(:,ien)=sca_map.*fine ^ 2 / ( bohr * hartree * pi * vel )./(2*(pi^2)./enei2(ien)); %  Farfield scattering spectra for Angular
        %---- S pol
        ff_mas=ff_ma;
        ...
        CL_sca_m_CLas(:,ien)=sca_mas.*fine ^ 2 / ( bohr * hartree * pi * vel )./(2*(pi^2)./enei2(ien)); %  Farfield scattering spectra for Angular
        multiWaitbar( 'CL maps', ien / numel( enei2 ) );   
    end    
    multiWaitbar( 'CloseAll' ); %  Close waitbar 
%%  Plot-section 2: CL maps    
    figure; 
    delete('data/cl_map\*') % Delete old files    
    for ien = 1 : length(enei2)               
        ...
        title(name(6));
        %set( gca, 'YDir', 'norm' );
        saveas(gcf,['data/cl_map/' sprintf( '%04d', ien ) 'CL_' num2str(ene2(ien)) 'eV' '.png'])
        saveas(gcf,['data/cl_map/' sprintf( '%04d', ien ) 'CL_' num2str(ene2(ien)) 'eV' '.fig'])
    end 
    close
    imagefiles = dir('data/cl_map/*.png') ; % Find files in folder       
    nfiles = length(imagefiles); % Number of files found
    for ii=1:nfiles
        currentfilename = imagefiles(ii).name;
        currentimage = imread(sprintf(['data/cl_map/' currentfilename]));
